                    ...
                    {props.professores.map(professor => (
                        <ItemLista key={professor.id}>
                            ...
                            <Informacoes>
                                ...
                                <Valor>{FormatadorService.valorMonetario(professor.valor_hora)} / hour</Valor>
                                <Descricao>{FormatadorService.limitarTexto(professor.descricao, 200)}</Descricao>
                                <Button onClick={() => props.onSelect(professor)} sx={{ width: '70%' }}>Book class with {professor.nome}</Button>
                            </Informacoes>
                        </ItemLista>
                    ))}
                </ListaStyled>
            ) : (
                    <ListaVazia>No item found</ListaVazia>
            )}
        </div>
    )
}
